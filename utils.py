...
class FPSCounter:
    """Exponentially-smoothed FPS counter, call .tick() once per frame."""
    ...
    def __init__(self, interval: int = 5):
        ...
        if self._using_gpu:
            try:
                ...
                pass  # pynvml not installed -> fall back to nvidia-smi
...
def draw_resources(frame: np.ndarray, res: dict) -> np.ndarray:
    """
    Draws a small dark panel in the top-right corner:
      FPS  xx.x
      GPU  xx%      (or CPU  x.x core, if no CUDA device)
      RAM  xxxMB
      VRAM xxxMB    (GPU only)
    """
    h, w = frame.shape[:2]
...
def make_output_path(output_dir: str, source_name: str, suffix: str) -> Path:
    """Build e.g. outputs/photo_detected.jpg, creating the output folder if needed."""
